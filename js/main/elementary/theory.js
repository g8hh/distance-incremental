...
function unlockStrings() {
	...
	if (player.mlt.times.eq(0)) if (!confirm("您确定要解锁弦吗？花费的学说点数无法返还！")) return
...
function unlockPreons() {
	...
	if (player.mlt.times.eq(0)) if (!confirm("您确定要解锁前子吗？花费的学说点数无法返还！")) return
...
function unlockAccelerons() {
	...
	if (player.mlt.times.eq(0)) if (!confirm("您确定要解锁加速子吗？花费的学说点数无法返还！")) return
...
function unlockInflatons() {
	...
	if (player.mlt.times.eq(0)) if (!confirm("您确定要解锁暴胀子吗？花费的学说点数无法返还！")) return
